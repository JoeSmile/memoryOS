...
- **WHEN** client repeats cancel for the same `stream_id`
...
- **THEN** the client aborts the in-flight chat request and issues a non-blocking cancel API request with that `stream_id`
