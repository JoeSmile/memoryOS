...
- [ ] 本机安装 OpenSpec CLI（`npm i -g @fission-ai/openspec`）
- [ ] 仓库根 `openspec init` + `openspec update`（生成 `openspec/`）
- [ ] `openspec/config.yaml` 写入 MemoryOS 上下文（Monorepo、`apps/api` Python、`pnpm` 脚本、FE/BE 文档链接）
- [ ] Cursor 可执行 `/opsx:propose`（试一次 `ep03-data-storage`）

## Story 0.2 Superpowers 接入

- [ ] Cursor 安装 Superpowers（Plugin Marketplace 或官方 skills 路径）
- [ ] 团队约定：P0 Story 开工前 **brainstorm 或 plan**（记录在 OpenSpec proposal 或 issue）
- [ ] 在 `CONTRIBUTING.md` 或 PR 模板中链接 [ai-collab-stack.md](../../tech/ai-collab-stack.md)

## Story 0.3 OpenSpec 与 EP03 首次闭环

- [ ] `/opsx:propose "ep03-data-storage"`：proposal、design、tasks 与 [EP03](./EP03-data-storage.md) 对齐
- [ ] 实现 EP03 Story 3.1–3.2 时按 `tasks.md` 勾选（可用 `/opsx:apply`）
- [ ] 完成后 `/opsx:archive`，主 spec 与 `docs/database.md` 一致

## Story 0.4 Harness 基线

- [ ] 创建 `apps/api/tests/harness/` + `README.md`
- [ ] L1：`test_health_contract.py`（状态码、`code/message/data` 结构）
- [ ] 根目录或 `apps/api` 文档写明：`pytest tests/harness` 命令
