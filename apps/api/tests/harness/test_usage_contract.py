...
async def test_chat_completion_records_mock_token_usage():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        async with AsyncSessionLocal() as session:
            ...
            assert row.prompt_tokens == mock_model.MOCK_TOKEN_USAGE["prompt_tokens"]
            assert row.completion_tokens == (
                mock_model.MOCK_TOKEN_USAGE["completion_tokens"]
            )
...
async def test_chat_completion_returns_42902_when_daily_quota_exceeded(monkeypatch):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        assert body["message"] == "token_quota_exceeded"
