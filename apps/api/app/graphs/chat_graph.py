...
def _build_rag_only_graph():
    """EP04/EP06: trim_history → load_user_memories → retrieve → call_model → END."""
    ...
    builder.add_edge("trim_history", "load_user_memories")
    builder.add_edge("load_user_memories", "retrieve_knowledge")
...
def _build_react_graph():
    """EP05/EP06: trim_history → load_user_memories → retrieve → call_model ↔ execute_tools."""
    builder = StateGraph(ChatState)
    builder.add_node("trim_history", trim_history)
    builder.add_node("load_user_memories", load_user_memories)
    builder.add_node("retrieve_knowledge", retrieve_knowledge)
